...
static const char kWindowTitle[] = "Star Fox (Recompiled)";
...
int main(int argc, char** argv) {
  ...
  host_report_init("Star Fox", SNESRECOMP_BUILD_VERSION);
  ...
  if (!rom_resolved_by_launcher) {
    ...
    if (!snesrecomp_launcher_resolve_rom_sha256(la_argc, la_argv, rom_path_buf,
                                                sizeof(rom_path_buf), kStarFoxSha256)) {
  ...
  // Initialize debug server
  {
    ...
    /* Star Fox uses debug port 4381 so sibling recomp projects can run
...
error_reading:;
  ...
  // The runner typically loads starfox.sfc from cwd via the asset pipeline
  // (argv[0] is usually NULL), so use that name when
  // argv[0] was not supplied.
  if (g_config.enable_snes9x_oracle) {
    extern int snes_oracle_init_default(const char *rom_path);
    const char *rom_path = (argv[0] && *argv[0]) ? argv[0] : "starfox.sfc";
    ...
    static const char *kReason =
        "Star Fox oracle comparisons require synchronized input and "
        "Super FX-aware save states. The current snes9x backend starts "
        "independently from boot, so enabling it would compare unrelated "
        "game moments. Disabled in config.ini until synchronized replay "
        "or save-state parity exists.";
    snes_oracle_set_disabled_by_game(kReason);
    fprintf(stderr,
        "\n=== snes9x oracle DISABLED for Star Fox ===\n"
        "Reason: %s\n"
  ...
  if (g_config.enable_audio) {
     ...
     * titles in snesrecomp f022984; that change never reached Star Fox, so it
     * opened the device 40 samples/s slower than the guest produces and walked
     * the output ring to its cap in under three minutes of play. */
